...
public class LoginPageTest {
    ...
    public void testLoginWithEmptyUsername() {
        ...
        Assert.assertEquals(loginPage.errorMsg(), "*Username is Empty");
    }

    @Test(priority = 7)
    public void testLoginWithEmptyPassword() {
        loginPage.loginToApplication("rahul", "");
        Assert.assertEquals(loginPage.errorMsg(), "*Password is Empty");
    }

    @Test(priority = 8)
    public void testLoginWithInvalidPassword() {
        loginPage.loginToApplication("rahul", "rahul2021");
        Assert.assertEquals(loginPage.errorMsg(), "*Password didn't match");
    }

    @Test(priority = 9)
    public void testLoginWithInvalidUsername() {
        loginPage.loginToApplication("raul", "rahul@2021");
        Assert.assertEquals(loginPage.errorMsg(), "*Username didn't match");
    }

    @Test(priority = 10)
    public void testLoginWithInvalidUsernameAndPassword() {
        loginPage.loginToApplication("rahl", "rahul2021");
        Assert.assertEquals(loginPage.errorMsg(), "*Username or Password didn't match");
    }
